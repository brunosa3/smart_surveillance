...
class FaceRecognition:
    def who_is_it(self, image_path, FaceNet, knn, SGDC, thr=0.75, plot=False,res_model="espcn", zoom=4):
...
#         print(face.shape)
        ...
        if face is None:
            return None, ["No face visible"], None, None, ["No face visible"]
#         plt.imshow(face)
...
#         face = self.resize(face[:,:,0], 160, 160)
        ...
        OUTPUT_face = "/".join(image_path.split("/")[:1] + ["FaceNet_output"] + image_path.split("/")[2:-1])
        Path(OUTPUT_face).mkdir(parents=True, exist_ok=True)
        tmp = OUTPUT_face + "/" + image_path.split("/")[-1]
        plt.imsave(tmp, face)
        
        # Compute the target "encoding" for the image
        encoding =  self.img_to_encoding(tmp, FaceNet)

        res = self.find_nearest([int(k) for k in knn.keys()], resolution)
        print("nearest resolution {}".format(res))
        
        knn_ident, knn_dist, knn_status = knn[res].predict(encoding)
        
        if knn_status == "distance":
            print("KNN used class size to re-weight the neigbors")
        else:
            print("KNN did not use class size to re-weight the neigbors")
            
        n,w = knn_dist[0]
        knn_dist = list(zip(n,w))
        knn_dist_ind = [x for x, y in enumerate(knn_dist) if y[0] ==  knn_ident]

        knn_min_dist = 1/np.array(knn_dist)[knn_dist_ind][:,1].astype(float).mean()

        ### END CODE HERE

        if knn_min_dist < 1/thr:
            print("Not in the database.")
            knn_identity = ["unknown"]
            knn_min_dist = None
        else:
            print ("it's " + ' '.join(knn_ident) + ", the distance is " + str(knn_min_dist))
            
        
        proba = SGDC[res].predict_proba(encoding.reshape(1,-1)) 
        ...
        if ind[1] != []:
            ...
            SGDC_ident = [SGDC[res].classes_[ix]]
            SGDC_min_dist = proba[0,ix]
            
#             SGDC_ident = SGDC[res].classes_[ind[1]]
#             SGDC_min_dist = proba[ind]
        else:
            SGDC_ident = ["unknown"]
            SGDC_min_dist = None
        print("knn - identity: {}, dist: {}".format(knn_ident, knn_min_dist))
        print("SGDC - identity: {}, dist: {}".format(SGDC_ident, SGDC_min_dist))
        
        return knn_min_dist, knn_ident, encoding, SGDC_min_dist, SGDC_ident
